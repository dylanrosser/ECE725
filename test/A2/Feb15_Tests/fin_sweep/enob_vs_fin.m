...
csvfiles = dir('*.csv');
enobs = []
fin = str2double(erase(extractfield(csvfiles, 'name'), 'kHz.csv'))
count = 1;

for file = csvfiles'
    num_bits = get_enob(string(file.name), fin(count)*1e3);
    fprintf(1, 'file is %s %d \n', file.name, fin(count)*1e3);
    enobs = [enobs, num_bits];
    count = count+1;
    
end

[fin_sorted, idx] = sort(fin);
enobs_sorted = enobs(idx);
semilogx(fin_sorted(1:6), enobs_sorted(1:6));
xlabel('f_{in} (kHz)');
ylabel('ENOB')
title('ENOB vs. Input Frequency');

% read into matlab variables
function ge = get_enob(file, f_in)
